...
app.post('/api/claude', aiLimiter, async (req, res) => {
  ...
  try {
    ...
    if (!text) return res.status(502).json({ error: 'Claude returned empty response.' });
    res.json({ text });
...
app.post('/api/grok', aiLimiter, async (req, res) => {
  ...
  try {
    ...
    if (!text) return res.status(502).json({ error: 'Groq returned empty response.' });
    res.json({ text });
...
async function _groqFallback(req, res, system, safeMessage) {
  try {
    ...
    const data = await upstream.json();
    const text = data?.choices?.[0]?.message?.content || '';
    if (!text) return res.status(502).json({ error: 'Groq fallback returned empty response.' });
    res.json({ text });
  } catch (_err) {
...
co2Level: high = meat, petrol vehicles, flights, single-use plastic; medium = dairy, electronics, packaged food; low = vegetables, public transport, renewable energy.`;
  ...
  try {
    ...
    res.json({
      ...
      co2Level: validLevels.includes(parsed.co2Level) ? parsed.co2Level : 'medium',
...
app.listen(PORT, () => {
  ...
  process.stdout.write(`Groq:   ${process.env.GROK_API_KEY   ? 'set' : 'MISSING'}\n`);
  process.stdout.write(`Gemini: ${process.env.GEMINI_API_KEY ? 'set' : 'MISSING'}\n`);
  _startKeepAlive();
});
