...
const allowedOrigins = [
  ...
  "https://fancy-cannoli-d8257b.netlify.app",
  "http://127.0.0.1:5500",
  "http://localhost:3000",
].filter(Boolean);
...
app.use(
  cors({
    origin: (origin, callback) => {
      // Allow Render health checks and direct API calls (no origin)
      if (!origin) return callback(null, true);
      // Allow your Netlify frontend
      if (allowedOrigins.includes(origin)) return callback(null, true);
      // In production log it, but still allow for now
...
app.post("/api/grok", aiLimiter, async (req, res) => {
  ...
  if (!process.env.GROK_API_KEY) {
    return res
      .status(503)
      .json({ error: "Grok API key not configured on server." });
  }
